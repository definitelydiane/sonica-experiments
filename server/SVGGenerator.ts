...
export class SVGGenerator {
  ...
  public fontSize(s: number): SVGGenerator {
    this._fontSize = s;
    return this;
  }
  ...
  public generate(): string {
    ...
    lines.push(
      `<g
				id="staff"
				stroke="${this._color}"
				stroke-width="${this._staffSpaceHeight * 0.13}"
				>`.trim()
    );
    for (let i = 0; i < 5; i++) {
      // 5 lines in staff
    ...
    lines.push(
      `<line
             ...
             this._staffMarginT -
             this._staffHeight -
             (this._staffSpaceHeight * 0.13) / 2
           }"
					 y2="${this._staffMarginT + (this._staffSpaceHeight * 0.13) / 2}"
					 stroke="${this._color}"
					 stroke-width="${this._staffSpaceHeight * 0.16}"/>`.trim()
    );
  ...
  private _createGlyphNode(
    ...
    this._minHeight = Math.max(
      this._minHeight,
      glyph.height * this._staffSpaceHeight
    );
